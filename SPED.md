...
- Codex 0.125.0 wire-format gap: the published JSON schema for `InitializeParams`,
  `ThreadStartParams`, and `TurnStartParams` defines `DynamicToolSpec` but does not reference it
  from any request property. Implementations targeting 0.125.0 MUST therefore rely on an
  out-of-band registration path (e.g. MCP server registration) for codex to know about a
  client-side tool. The `item/tool/call` handler MUST still be wired so the session does not
  stall if codex is configured externally to invoke the tool — under the no-advertisement gap the
  handler is dormant for an unconfigured codex.
